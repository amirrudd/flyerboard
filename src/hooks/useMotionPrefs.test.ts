...
describe('useMotionPrefs — slideOver', () => {
    it('slides in from the right (24px, 200ms) with a reversed ease-in exit', () => {
        ...
        expect(props.exit.x).toBe(24);
        expect(props.exit.transition.duration).toBe(0.15);
    });
    ...
    it('collapses to zero duration and zero offset under prefers-reduced-motion', () => {
        ...
        expect(props.exit.transition.duration).toBe(0);
    });
});
