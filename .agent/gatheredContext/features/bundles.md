...
**Last Updated**: 2026-07-17 (unified feed: bundle cards arrive server-interleaved via feed.getFeed)
...
- `AdsGrid` renders a `"bundle"` `FeedEntry` kind (`{ kind: "bundle", card }`) +
  `onBundleClick` prop; since the unified feed (2026-07-17) bundle cards arrive
  pre-interleaved in `api.feed.getFeed`'s page — `getActiveBundleFeedCards` was DELETED
  (home feed was its only caller; `feed.hydrateBundleCard` preserves its exact card shape
  and the "despawn below 2 live members" rule).

### Unified feed replaced the pop-in fix (2026-07-17)
The 2026-07-16 `auxCardsPending` skeleton-gate patch (bundle/sale cards popping into the
feed a round-trip after ads painted) is GONE, along with the client-side three-way merge:
`feed.getFeed` returns one server-interleaved page, feature flags evaluated server-side,
so there is exactly one loading state by construction. Any new feed card type belongs in
`convex/feed.ts` as another merged stream — do NOT add an independent client `useQuery`
feed source again.
