...
| [Fuzzy search](ideas/fuzzy-search.md) | Ready | 1 | 4 | 2 | **4** | Survived red-team 2026-08-15 and got *cheaper*: Convex ORs search terms, so synonym expansion is a string join — no merging. ~2 days: `searchText` (doubled title + description), AU synonym dict, `searchLogs`, zero-state redesign. Ship all four together. Vectors deferred on transactionality grounds. |
| [Proximity-ranked feed](ideas/proximity-ranked-feed.md) | Exploring | 4 | ? | 3 | **2** | **Red-team overturned the initial BUILD verdict 2026-08-15.** Location defaults to unfiltered (premise unproven), no analytics to measure any KPI, distance isn't orderable in Convex, contradicts locked Boost rules. Survives as a ~1-day soft-filter fix. Ring test blocked on Convex auth. |
