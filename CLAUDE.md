...
- **Home feed**: one paginated query, `api.feed.getFeed` (`convex/feed.ts`) — a convex-helpers `mergedStream` interleaving ads, standalone Bundle cards, and Moving Sale cards on the shared `bumpedAt` sort key, with feature flags evaluated server-side. Category and search feeds still go through `ads.getAds`. Pattern details: `gatheredContext/infrastructure/database.md` ("Unified home feed").
...
`convex/_generated/ai/guidelines.md` (generated by `npx convex ai-files`, kept current by `convex dev`) enforces the modern Convex function syntax (object form with `args`, `returns`, `handler`) and validator usage. Apply these when writing new Convex functions. It replaced the old `.cursor/rules/convex_rules.mdc`.
