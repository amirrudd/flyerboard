...
## /messages redesign (2026-07-11, PR #289 — phases 1-4 of .agent/plans/mobile-chat-redesign.md)

- **Routes**: `/messages` (inbox: All/Selling/Buying pills, `?flyer=` chip), `/messages/archived` (unarchive + guarded both-sides bulk delete), `/messages/:chatId` (full-screen thread; body portal on <md, BottomNav + header hidden there — `/messages/archived` is NOT a thread and keeps both). All in `src/pages/MessagesPage.tsx`.
...
2. **`src/features/ads/AdMessages.tsx`** — seller's per-ad inbox (master-detail: conversation list + thread). **Since Phase 4 reached ONLY via legacy `/dashboard?messages=<adId>` deep links** — the My Flyers button now goes to `/messages?flyer=<adId>`. Cmd/Ctrl+Enter sends (textarea). Protected behaviors doc: `ADMESSAGES_BEHAVIOR.md`. Contains a `ReportModal` that is **unreachable** — nothing ever sets `showReportModal(true)`. Retirement pending Phase 6 confirmation.
